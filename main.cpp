...
const char className[]="gameForm"; //им€ класса окна
HWND main_window; //дескриптор главного окна

game::field *gamefield; //!обь€вл€ем класс игры

//прототипы функций
LRESULT CALLBACK WinProc(HWND, UINT, WPARAM, LPARAM); //оконна€ функци€ дл€ обработки событий окна
BOOL OnCreate(HWND, void*);
void OnPaint(HWND);
void OnDestroy(HWND);
void OnClick(HWND, BOOL, int, int, UINT);
void OnMouseMove(HWND, int, int, UINT);
RECT arrowArea; //зона рисовани€ стрелки
void ShowGameInfo();


int WINAPI WinMain(HINSTANCE hThisInstance /* дескриптор приложени€ */,
                   HINSTANCE, LPSTR cmdLine, int showStyle /* режим показа окна */)

{
	//определ€ем параметры игры
	std::stringstream s;
	s<<cmdLine;
	s>>M>>N;
	if (M<3) M=3;
    ...
    wc.hInstance=hThisInstance; //дескриптор приложени€
    wc.lpszClassName=className; //им€ класса
    wc.lpfnWndProc=WinProc; //функци€ обработки событий
    ...
    //—оздаем окно
    main_window=CreateWindow( className, " ольца наркоманов 2", /*им€ класса и заголовок */
							  WS_OVERLAPPEDWINDOW, 35, 40, gamefield->GetWindowWidth()+10, gamefield->GetWindowHeight()+27, /* стиль окна, позици€ и размеры */
                              HWND_DESKTOP, NULL /* No menu */, hThisInstance /*дескриптор приложени€*/, NULL /*доп.данные*/
                             );
...
{
    delete gamefield; //удал€ем игру
	PostQuitMessage(0);
}

void ShowGameInfo()
{
	char msg[80]="—оберите все ";
	char tmp[4]; _itoa(N, tmp, 10);
	strcat(msg, tmp);
	strcat(msg, " колец на последней (справа) башне!");
	MessageBox(main_window, msg, " ольца наркоманов 2", MB_OK|MB_ICONINFORMATION);
}

//////////////////////////////////////////////////////////////////////////


static bool fSelectFirstTower=false;  //выделена перва€ башн€ дл€ перемещени€
...
void OnClick(HWND hwnd, BOOL fdbclick, int x, int y, UINT keyFlugs)
{
	...
	if (position){
		if (!fSelectFirstTower) {
			...
			gamefield->setSelectTower(position); //выделенна€ башн€
			//определ€ем зону перерисовки
			...
			if (gamefield->Move(fromTower-1, toTower-1)) {
				...
				gamefield->setSelectTower(0); //сн€ть выделение
				gamefield->setTargetTower(0);

				//тест на выиграш
				if (gamefield->is_win()) {
					char msg[80]="ѕоздравл€ем! ¬ы прошли игру за "; msg[12]='\n';
					char tmp[4]; _itoa(gamefield->getTurnNumber(), tmp, 10);
					strcat(msg, tmp);
					strcat(msg, " ходов.");

					MessageBox(main_window, msg, "¬ы выиграли.", MB_OK|MB_ICONINFORMATION);
				};
			}
			else {
				fSelectFirstTower=true;
				gamefield->setSelectTower(position); //новое выделение
				fromTower=position; //!нова€ исходна€ позици€
			}
